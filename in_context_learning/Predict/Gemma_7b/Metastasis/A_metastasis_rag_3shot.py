...
for index, row in tqdm(extracted_notes.iterrows(), total=extracted_notes.shape[0], desc="Processing notes"):
    if row['LLM_class'] == 'Pending':
        ...
        while not matched_response and count < 5 and not error_occurred:
...
# %%
extracted_notes.to_csv(f"/results/p2/p1_results_023x2_few_shot_{RANDOM_SEED}_new.csv", index=False)
