...
# Load the dataset
extracted_notes = pd.read_csv('/subset_data/extracted_discharge_notes_insulin_p1.csv')
...
Task: You are a classification bot. Classify the use of insulin from patient clinical notes. Respond only with the following numerical codes based on the notes provided:
...
1. Do not provide reasons or explanations  for your classification.
...
4. Only provide a single short numerical code as the response you must follow this rule.
    ...
    if row['LLM_class'] == 'Pending':
        ...
        while not matched_response and count < 10 and not error_occurred:
            ...
            try:
                ...
                #print(f"Response from model: {hf_response}")
...
# Save final results
extracted_notes.to_csv(f"/results/P1/A10_insulin_llama70_31_{RANDOM_SEED}.csv", index=False)