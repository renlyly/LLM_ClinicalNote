...
import sys
# Load the dataset
extracted_notes = pd.read_csv('/subset_data/extracted_discharge_notes_hypertension_p1.csv')
    ...
    if row['LLM_class'] == 'Pending':
        ...
        while not matched_response and count < 10 and not error_occurred:
            ...
            try:
                ...
                #print(hf_response) 
                match = stage_pattern.search(hf_response)
                if match:
...
# Save final results
extracted_notes.to_csv(f"/results/P1/A10_hypertension_llama70_21_{RANDOM_SEED}.csv", index=False)