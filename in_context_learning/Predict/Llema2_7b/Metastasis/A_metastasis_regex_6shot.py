...
extracted_notes = pd.read_csv('subset_data/extracted_discharge_notes_p1.csv')
...
for index, row in tqdm(extracted_notes.iterrows(), total=extracted_notes.shape[0], desc="Processing notes"):
    if row['LLM_class'] == 'Pending':
        ...
        while not matched_response and count < 15 and not error_occurred:
...
# Save final results
extracted_notes.to_csv(f"/results/P1/p1_results_llama70_31_{RANDOM_SEED}.csv", index=False)
